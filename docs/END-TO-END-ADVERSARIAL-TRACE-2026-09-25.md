...
9. Travel progression. Firestore permits Elena's phone to publish only Travel telemetry. The authenticated server owns the status ladder: it verifies assignment and predecessor state, requires fresh presence near pickup for arrival and near destination for completion, and preserves Teen pickup verification. The phone cannot author a payable completion or alter fare, Booker, Traveler, payment, operator assignment or rating.
...
Smart Travel is a journey composed of up to two real car Travels around a transit leg. Each car Travel has its own Operator, Travel Number, payment and settlement. Leg 2 may reference only a paid first leg owned by the same Booker. The coordinated platform fee is calculated over the journey's car fare while funding two transaction-cost units when two car Travels are charged; leg 2 carries only the incremental fee not already carried by leg 1. The Smart Travel preview uses that same two-charge calculation. Journey-level economics must preserve two contribution units when there are two charged car legs. Transit fare is not represented as an American Rider charge unless the platform actually collects it.
